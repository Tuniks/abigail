...
public class FireInteraction : MonoBehaviour
{
    ...
    void Update()
    {
        if (BeforeInteraction != null && player != null) 
        {
            ...
            if (Input.GetKeyDown(KeyCode.E) && distance <= interactionRadius) //if the player pressess E, the curtain interaction hasn't happened and the distance is within the interactino radisu then set after interaction sprite and chnage the bool
            {
